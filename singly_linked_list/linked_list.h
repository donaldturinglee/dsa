...
void LinkedList<T>::removeTail()
{
    ...
    // the element next to the previous node
    Node<T>* node = head->next;

    // Traverse the elemetns until
    // the last element
    ...
    // the previous element now becomes the tail
    // so the next pointer of the previous node
    // point to nullptr
    prev->next = nullptr;
    tail = prev;
